...
def classementBorda(data, title, is_merged):
...
def plot_borda(data, title, is_merged):
    # Barplot
    plt.figure(figsize=(10, 6))
    ...
    plt.title(f'Classement Borda : {title}')
    plt.legend()
    plt.xticks(np.arange(1, len(scores) + 1))
    plt.grid(True)
    if is_merged:
        plt.savefig(f'classements_des_saisons_en_fonction_des_methodes_sur_les_courses/borda_{title}.png')
    else:
